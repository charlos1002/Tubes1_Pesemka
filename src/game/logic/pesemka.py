...
class Pesemka(BaseLogic):
    ...
    def objects_by_type(self, board: Board, type_name: str) -> List[GameObject]:
        return [o for o in board.game_objects if o.type == type_name]

    def grid_distance(self, a: Position, b: Position) -> int:
        return abs(a.x - b.x) + abs(a.y - b.y)

    def refresh_teleporters(self, board: Board) -> None:
    ...
    def distance_via_tp(self, src: Position, dst: Position) -> int:
    ...
    def nearest_tp(self, src: Position) -> Optional[GameObject]:
        if not self.teleporter_pairs:
            return None
    ...
    def choose_optimal_target(self, bot: GameObject, board: Board) -> Position:
